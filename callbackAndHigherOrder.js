...
// CODE HERE
const multiply = (num1, num2, answer) => answer(num1 * num2)
...
// CODE HERE
const last = (array, callback) => callback(array[array.length - 1]) // array[0] also gives you the same result but it's less dynamic
...
const uniq = (array, callback) => {
  const uniqNames = array.filter((callback, index)=>{
    return names.indexOf(callback) === index;
  });
  callback(uniqNames)
}
...
const nameIndex = (arr) => {
  for(let i = 0; i < arr.length; i++){
    console.log(`The item at ${[i]} is ${arr[i]}.`)
  }
}
